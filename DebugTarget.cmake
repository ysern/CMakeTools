function(print_target_property target prop)
	...
	if(NOT value STREQUAL "value-NOTFOUND")
		message(STATUS "--  Property ${prop}: ${value}")
	endif()
endfunction()

function(print_target_property_expand target prop)
    if (${prop} MATCHES "^.*<CONFIG>.*")
        ...
        foreach(config ${config_list})
            string(REGEX REPLACE "<CONFIG>" "${config}" prop_expanded "${prop}")
            print_target_property(${target} ${prop_expanded})
        endforeach()
    elseif (${prop} MATCHES "^.*<LANG>.*")
        ...
        foreach(lang ${lang_list})
            string(REGEX REPLACE "<LANG>" "${lang}" prop_expanded "${prop}")
            print_target_property(${target} ${prop_expanded})
        endforeach()
    elseif (${prop} MATCHES "^.*<NAME>.*")
        string(REGEX REPLACE "<NAME>" "${target}" prop_expanded "${prop}")
        print_target_property(${target} ${prop_expanded})
    else()
...
# Function to print target properties for debugging
function(print_target_properties target)
    ...
    # Loop through properties and get their values
    foreach(prop ${property_list})
        print_target_property_expand(${target} ${prop})
    endforeach()
